...
const getData = async (myReqValue)=>{
    ...
    div.innerHTML = `
             <h2>Word : <span>${jasonDat[0].word}</span></h2> 
             <p> Meaning : <span> ${jasonDat[0].meanings[0].definitions[0].definition} 
              <p> Example : <span> ${jasonDat[0].meanings[0].definitions[0].example} 
             <p>${jasonDat[0].meanings[0].partOfSpeech}</p>
            <p>Synonyms :<span>${jasonDat[0].meanings[0].synonyms}</span></p>         
            <audio controls>
            <source src="${jasonDat[0].phonetics[0].audio}" type="audio/ogg">
